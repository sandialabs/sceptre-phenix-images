...
function onResponse(req, resp) {
    log("[" + G("http-resp-dump") + "] " + BLACK_RED(resp.Scheme) + " " + resp.Client + " " + BLACK_BLUE(resp.Method) + " " + GREY(resp.Scheme + "://") + Y(resp.Hostname) + resp.Path + (resp.Query != "" ? GREY("?" + resp.Query) : ""));

    dumpHeaders(resp);
    //if (resp.Query.length > 0) {
    //	console.log("QUERY > 0\n");
    //  dumpQuery(resp);
    //}
    //console.log("FINISHED DUMPING QUERY\n");

    if (resp.ContentType.indexOf("text/plain") != -1) {
        dumpPlain(resp);
    } else if (resp.ContentType.indexOf("application/x-www-form-urlencoded") != -1) {
        dumpForm(resp);
    } else if (resp.ContentType.indexOf("application/json") != -1) {
        dumpJSON(resp);
	writeHex(resp);
    } else if (resp.ContentType.indexOf("application/x-javascript") != -1) {
        console.log("GOT JAVASCRIPT PACKET\n");
    } else if (resp.ContentType.indexOf("image/svg+xml") != -1) {
        console.log("RECEIVING NEW VIEW\n");
    } else {
        dumpRaw(resp);
    }
}
