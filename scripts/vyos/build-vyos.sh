...
FILE=\$(ls vyos/vyos-build/build/*.qcow2)
...
# copy miniccc binary
cp /root/vyos/miniccc \$ROOTFS/usr/local/bin/miniccc
...
function inject_miniccc() {
    ...
    docker_build
    cp $1 $SCRIPT_DIR/miniccc
    docker run --rm -it \
        -v /dev:/dev \
        -v /lib/modules:/lib/modules \
        -v "$(pwd)":/root/vyos \
        -w /root --privileged --cap-add=ALL \
        inject-miniccc /root/vyos/miniccc
    rm $SCRIPT_DIR/miniccc
